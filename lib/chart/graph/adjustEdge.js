  ...
  graph.eachEdge(function (edge, idx) {
    ...
    if (linePoints[2] != null) {
      ...
      if (x0 === linePoints[1][0] && y0 === linePoints[1][1]) {
        isCircleEdge = true; // random deviation angle
        ...
        const diffLength = symbolSize * scale; // const circleEdgeLength = linePoints[2].r * symbolSize / 40

        linePoints[0] = [x0 + diffLength * Math.cos(angle + diffAngle), y0 + diffLength * Math.sin(angle + diffAngle)];
        linePoints[1] = [x0 + diffLength * Math.cos(angle - diffAngle), y0 + diffLength * Math.sin(angle - diffAngle)]; // linePoints[2] = [x0 + circleEdgeLength * Math.cos(angle), y0 + circleEdgeLength * Math.sin(angle)]
        ...
        const p1_x = cx2 - xk * diff * sinA,
              p1_y = cy2 - xk * diff * cosA;
        const p2_x = cx2 + xk * diff * sinA,
              p2_y = cy2 + xk * diff * cosA;
